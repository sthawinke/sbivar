...
#' marks(PPP, drop = FALSE) <- data.frame(feature = marks(PPP)) # Make sur marks is a dataframe
#' resMoransI <- sbivar(X = PPP, Y, Cx, Ey, method = "Moran's I")
#' resGAMs <- sbivar(X = PPP, Y, Cx, Ey, method = "GAMs")
