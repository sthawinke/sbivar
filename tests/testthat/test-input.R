...
test_that("SbivarSingle throws errors for correct input", {
    ...
    expect_error(sbivarSingle(X, Y, Cx, Ey, method = "GAMs", families = list(gaussian(), gaussian())))
    expect_error(sbivarSingle(X, Y, Cx, Ey, method = "GAMs", families = list("X" = gaussian(), "Y" = mean)))
