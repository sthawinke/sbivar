...
htmltools::img(
    src = knitr::image_uri("../inst/Sbivar.png"),
...
Furthermore, it is highly recommended to install a matrix library such as OpenBLAS \url{http://www.openmathlib.org/OpenBLAS/}, that employs threading for key matrix manipulations used throughout the package.
...
We analyse this single sample using bivariate Moran's I. By default, three different $\eta$ parameters are considered for constructing the elements of the weight matrix: `r format(eval(formals(sbivar:::sbivarSingle)$etas))`. We choose to normalize both modalities to relative abundances (see ?normMat). 
...
We see a clear positive relationship.
...
Another feature is found most significant by GAMs, this one with a negative relationship. For GAMs, we can also plot the corresponding spline fit, with the contributions to the correlation `r round(gamRes$result[gpGam<- rownames(gamRes$result)[1], "corxy"], 3)`:
...
For the multi-image case, we can also use bivariate Moran's I as measure of spatial association, with a nearest-neighbour (nn) weighting scheme for illustration (if the required _RANN_ package is available). For computational reasons, we limit calculations to the first 500 observations:
