...
We analyse this single sample using bivariate Moran's I, choosing log-normalization (see _?logNorm_) for both modalities:
...
For the multi-image case, we can also use bivariate Moran's I as measure of spatial association, with a nearest-neighbour (nn) weighting scheme for illustration (if the required _RANN_ package is available):

```{r multiMoran}
Vicari = lapply(Vicari, function(x) lapply(x, function(y) y[1:200, ]))
multiMoranRes = sbivar(Vicari$TranscriptOutcomes, Vicari$MetaboliteOutcomes, normX = "rel", normY = "rel",
...
```{r multiLmms}
designDf = data.frame("mouse" = mouse, fac = c(1,2,1,2,1,2))
multiMoranLmms = fitLinModels(multiMoranRes, designDf, Formula = ~ fac + (1|mouse))
```
...
multiMoranResSE = sbivar(X = StxSElist, Y = MetSElist, assayX = "transcripts", verbose = FALSE,
                         assayY = "metabolites", method = "Moran", wo = "Gauss")
