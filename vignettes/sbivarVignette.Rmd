...
We analyse this single sample using bivariate Moran's I. By default, three different $\eta$ parameters are considered for constructing the elements of the weight matrix: `r eval(formals(sbivar:::sbivarSingle)$etas)`. We choose to normalize both modalities to relative abundances (see _?normMat). 
...
plotTopPair(moransIRes, X = singleStx, Y = singleMet, Cx = singleStxCoords, 
            Ey = singleMetCoords)
```
...
Plot the most significant pair (log-transformed data are shown, the original analysis is on raw data):
...
Another feature is found most significant by GAMs, likely because they smooth over the two holes in the spatial transcriptomics measurements (left plot), adding to the signal, whereas the holes do not contribute to the Moran's I. This can be an asset or a weakness of GAMs. For GAMs, we can also plot the corresponding spline fit, with the contributions to the correlation `r round(gamRes$result[gpGam<- rownames(gamRes$result)[1], "corxy"], 3)`:
...
multiMoranRes = sbivar(VicariMultiTest$TranscriptOutcomes, VicariMultiTest$MetaboliteOutcomes, normX = "rel", normY = "rel",
                            VicariMultiTest$TranscriptCoords, VicariMultiTest$MetaboliteCoords, 
                            method = "Moran", wo = if(require(RANN)) "nn" else "Gauss")
```

Next we plug the calculated Moran's I values into a linear model, with random effects for the individual mice, adn adding a bogus variable for illustrative purposes:
...
multiMoranResSE = sbivar(X = StxSElist, Y = MetSElist, assayX = "transcripts", verbose = FALSE,
...
```{r speanalyse}
if(reqadzk){
    GodfreyRes = sbivar(speStx, speMet, assayX = "Stx", assayY = "msi", 
