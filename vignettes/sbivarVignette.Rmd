...
Plot the most significant pair (log-transformed data are shown, the originall analysis is on raw data):

```{r plottoppairgam, fig.height = 4.5}
plotTopPair(gamRes, X = singleStx, Y = singleMet, Cx = singleStxCoords, 
            Ey = singleMetCoords, normX = "log", normY = "log")
```
...
For the multi-image case, we can also use bivariate Moran's I as measure of spatial association, with a nearest-neighbour weighting scheme for illustration (if the required _RANN_ package is available):

```{r multiMoran}
multiMoranRes = sbivar(Vicari$TranscriptOutcomes, Vicari$MetaboliteOutcomes, normX = "log", normY = "log",
...
```{r multiLmms}
designDf = data.frame("mouse" = mouse)
multiMoranLmms = fitLinModels(multiMoranRes, designDf, Formula = ~ (1|mouse))
```
...
resMAE = sbivar(MAE, assayX = "transcripts", assayY = "metabolites", 
                ...
                families = FamiliesVicari, method = "GAMs", normX = "log", normY = "log")
```
...
sbivarMultiSingleSE = sbivar(SEsingleStx, SEsingleMet, sample_id_x = "sample_id",
                             ...
                             method = "Moran's I", normX = "log", normY = "log")
...
if(reqadzk){
    GodfreyRes = sbivar(speStx, speMet, assayX = "Stx", assayY = "msi", 
                        normX = "log", normY = "log", verbose = FALSE)
