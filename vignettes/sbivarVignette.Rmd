...
author: Stijn Hawinkel and Wangjun Hu
output: 
...
```{r image, echo=FALSE}
htmltools::img(
    src = knitr::image_uri(system.file("Sbivar.png", package = "sbivar")),
...
param <- if (.Platform$OS.type == "unix") {
    ...
    MulticoreParam(nCores, stop.on.error = FALSE)
} else {
    # On windows, build vignette without multithreading
    SerialParam(stop.on.error = FALSE)
}
register(param)
```

On windows, once the package has been built, for multithreading use

```{r multiThreadWindows, eval = FALSE}
register(SnowParam(workers = nCores, type = "SOCK", stop.on.error = FALSE)) # or doParallel
```
...
```{r registerserial, eval = FALSE}
register(SerialParam(stop.on.error = FALSE))
```

Furthermore, it is highly recommended to install a matrix library such as [OpenBLAS](http://www.openmathlib.org/OpenBLAS/), [Intel MKL](https://www.intel.com/content/www/us/en/developer/tools/oneapi/onemkl-download.html) or [Apple's accelerate](https://developer.apple.com/documentation/accelerate), that employ threading and optimal memory usage for key matrix manipulations used throughout the package. Further instructions for OpenBLAS for windows users can be found [in this repo](https://github.com/david-cortes/R-openblas-in-windows).

# Example analysis on data by Vicari et al. (2024)
...
```{r plotcoordsVicari, fig.height = 5}
par(mfrow = c(2, 3))
plotCoordsMulti(Vicari$TranscriptCoords, Vicari$MetaboliteCoords, cex = 0.1)
par(mfrow = c(1, 1))
```

The alignment seems successful, notice that some tissue areas are only covered by one modality though. Next we explore the spatial distributions of the spot-wise sums, in our case these are the library sizes for the transcriptomics data and the total ion counts for the metabolomics data.

```{r plotSpotSums, fig.height = 7}
plotSpotSums(
...
These spot sums are usually considered to be technical artefacts, but since they can affect the statistical analysis it is good to be aware of them and their spatial distribution.
...
We analyse this single sample using bivariate Moran's I. We choose to normalize both modalities to relative abundances (see ?normMat). 
...
```{r headres}
head(moransIRes$result)
```

Plot the most significantly spatially associated gene-metabolite pair `r gpMoran <- rownames(moransIRes$result)[1]`:

```{r plottoppairmoran}
plotTopPair(moransIRes,
    X = singleStx, Y = singleMet, Cx = singleStxCoords,
    Ey = singleMetCoords, size = 1
)
```
...
An other feature is found most significant by GAMs, now with a positive relationship. For GAMs, we can also plot the corresponding spline fit, with the contributions to the correlation `r round(gamRes$result[1, "corxy"], 3)`:
...
The analysis results can be written to an external spreadsheet using the following one-liner, e.g. for the GAM results.

```{r sbivarSingleOut, eval = FALSE}
writeSbivarToXlsx(gamRes, file = tmpFile <- tempfile(fileext = ".xlsx"))
```
...
For the multi-image case, we can also use bivariate Moran's I as measure of spatial association, here with a nearest-neighbour (nn) weighting scheme for illustration (if the required _RANN_ package is available). For computational reasons, we limit calculations to the first 500 observations:
...
multiMoranLmms <- fitLinModels(multiMoranRes, designDf, Formula = ~ bogus + (1 | mouse))
```
...
# Other common input formats

Apart from matrices and lists, also other data objects from the Bioconductor universe can be analysed by _sbivar_.

## SpatialExperiment and MultiAssayExperiment
...
Next we call the _sbivar_ function with these as arguments, providing the names of the assays we want analysed. We set _verbose_ to FALSE to prevent printing output:
...
```{r plotTopSE}
plotTopPair(resSpatExp, X = StxSE, Y = MetSE, topRank = 2, size = 0.4)
```
...
```{r analyseMAE, eval = FALSE}
resMAE <- sbivar(MAE,
    assayX = "transcripts", assayY = "metabolites",
    experimentX = "Stx", experimentY = "Msi",
    families = FamiliesVicari, method = "GAMs", normX = "rel", normY = "rel"
)
```

### Multi-image

For the multi-image case, there are several options. One is to provide two lists of _SpatialExperiment_ objects to the _sbivar_ function. We construct such list here for illustrative purposes, and fit GAMs:
...
multiMoranLmmsSE <- fitLinModels(multiMoranResSE, designDf, Formula = ~ (1 | mouse))
...
multiMoranLmmsResSingleSE <- extractResultsMulti(multiMoranLmmsSingleSE, designDf)
```

## AnnData

AnnData is python data format often used to store spatial experiment results, which can also be read into R. An AnnData object is e.g. created by reading an '.h5ad' with the functionality of the _anndataR_ R-package. Here we adapted one a file from the [study](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/GZFCWC) by @Godfrey2025, keeping its structure but subsetting it to only necessary components and the 10 most abundant features per modality. For illustration, this file is included in the _sbivar_ package and can be read in (if the _anndataR_ and _rhdf5_ packages are installed) as:

```{r readh5ad, message = FALSE}
if (reqann <- (require(anndataR) && require(rhdf5))) {
    annDataObj <- read_h5ad(system.file("h5adData", "LC_170_subset.h5ad", package = "sbivar"), as = "HDF5AnnData")
}
```

Next, convert it to a _SingleCellExperiment_ object from the eponymous R-package:

```{r anndata}
if (reqann) {
    sceStx <- annDataObj$as_SingleCellExperiment()
}
```

Note that a _SingleCellExperiment_ object can only contain one modality at the time, here the transcriptomics. From there on, it is only a small step to a _SpatialExperiment_ object as accepted by _sbivar_. The only tricky part are the spatial coordinates, which have no fixed slot in the _AnnData_ object. Here, they have been stored in the "obsm" slot of the _AnnData_ object, which means they have ended up in the "reducedDims" slot of the _SingleCellExperiment_ object. In addition, we create a _SpatialExperiment_ object for the metabolomics data, which has oddly been stored in the unstructured ("uns") slot of the _AnnData_ object. To keep build times of this vignette low, we again only look at a subset of 10 features.
...
if (reqann) {
    ...
    rownames(speMet) <- make.names(annDataObj$uns[["mz_features"]][1:10])
...
More than another output format, point patterns as stored in the _spatstat::ppp_ format represent another data type that can be analysed by _sbivar_. More precisely, we can test for association between a point pattern, e.g. from a single-molecule assay such as seqFISH or Xenium, and a quantitative outcome, registered on the same coordinate framework (for tests between two point patterns, see the _smoppix_ package by @Hawinkel2026a). Available tests are bivariate Moran's I and GAMs. We illustrate the syntax on the built-in dataset by @Duchini2026. First Moran's I:

```{r duchini}
data(Duchini)
# Provide the point pattern first, constructed using the spatstat.geom::ppp function
duchRes <- sbivar(X = Duchini$Transcripts, Y = Duchini$ProteinOutcomes, Ey = Duchini$ProteinCoords, method = "Moran")
```

The plot overlays the point pattern as black squares on top of the quantitative modality in colour

```{r duchiniPlot}
plotTopPairPPP(duchRes, X = Duchini$Transcripts, Y = Duchini$ProteinOutcomes, Ey = Duchini$ProteinCoords)
```

Also GAMs are implemented, modelling the intensity of the point process using splines:

```{r duchiniGAM}
duchResGAM <- sbivar(X = Duchini$Transcripts, Y = Duchini$ProteinOutcomes, Ey = Duchini$ProteinCoords, method = "GAM")
plotGAMsTopResults(duchResGAM, X = Duchini$Transcripts, Y = Duchini$ProteinOutcomes, Ey = Duchini$ProteinCoords)
```

# Troubleshooting

## Sbivar takes forever

If _sbivar_ runs a long time for small datasets, check that you have OpenBLAS or equivalent installed, \textit{and that your R uses it}. Check with

```{r sessionInfoopenblas, eval = FALSE, echo = TRUE}
sessionInfo()
```

and look for the BLAS field. It should mention "openblas" or equivalent somewhere. Switching to OpenBLAS comes with a 20-50x speed increase for the rate-limiting steps in the _sbivar_ package.
...
> In fit.variogram(vg, vgm(model = vv, nugget = NA)) :
> No convergence after 200 iterations: try different initial values?

indicate variogram fitting did not reach convergence, but since this is just a nuisance quantity, a rough estimate is sufficient for inference and this warning can be safely ignored.

### GAM fitting

The following warning often recurs a couple of times when fitting many GAMS:

> step size truncated due to divergence 

It indicates that the _mgcv::gam()_ function had trouble modelling the spatial trend in the data for one or more features. This could, but need not always, lead to odd results. Always inspect significant pairs visually.

## gfortran not found on Mac

Mac users often encounter trouble installing the _sbivar_ package because of a missing _gfortran_ dependency. This is a common issue on Mac, unrelated to _sbivar_. Many solutions are available online, e.g. in [this blogpost](https://stackoverflow.com/questions/77836548/library-gfortran-not-found-when-installing-r-packages).
