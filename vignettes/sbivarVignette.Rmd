...
Plot the most significantly spatially associated gene-metabolite pair `r gpMoran <- rownames(topPairs(moransIRes))[1]`:
...
An other feature is found most significant by GAMs, now with a positive relationship. For GAMs, we can also plot the corresponding spline fit, with the contributions to the correlation `r round(topPairs(gamRes)[1, "corxy"], 3)`:

```{r plotGAM}
plotGAMsTopPair(gamRes,
    X = singleStx, Y = singleMet, Cx = singleStxCoords,
    Ey = singleMetCoords
)
```
...
multiMoranRes <- sbivar(VicariMultiTest$TranscriptOutcomes, VicariMultiTest$MetaboliteOutcomes,
    ...
    method = "Moran's I", wo = if (require(RANN)) "nn" else "Gauss"
)
```
...
topPairs(multiMoranLmmsRes, parameter = "Intercept")
```
...
plotTopPair(multiMoranLmmsRes, parameter = "Intercept",
...
resMAE <- sbivar(MAE,
    ...
    families = FamiliesVicari, method = "GAMs"
)
```
...
multiMoranResSE <- sbivar(
    ...
    assayY = "metabolites", method = "GAMs", verbose = FALSE
)
...
topPairs(multiMoranLmmsResSE, "Intercept")
```
...
if (reqann) {
    ...
    plotTopPair(GodfreyRes, X = speStx, Y = speMet)
}
```
