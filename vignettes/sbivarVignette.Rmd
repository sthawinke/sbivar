...
Furthermore, it is highly recommended to install a matrix library such as OpenBLAS \url{http://www.openmathlib.org/OpenBLAS/}, Intel MKL \url{https://www.intel.com/content/www/us/en/developer/tools/oneapi/onemkl-download.html} or Apple's accelerate \url{https://developer.apple.com/documentation/accelerate}, that employ threading for key matrix manipulations used throughout the package.
...
The Vicari data consist of six images which can be analysed jointly, but for didactical purposes we first analyse a single image here, the sample "V11L12-109_A1".
...
```{r plottoppairmoran, fig.height = 4.5}
...
In addition, we employ generalized additive models (GAMs), with a negative binomial outcome distribution for the transcriptome data, and a gamma outcome distribution for the metabolome data. A log-link is used in both cases.
...
Plot the most significant pair (normalized data are shown, the original analysis is on raw data):

```{r plottoppairgam, fig.height = 4.5}
plotTopPair(gamRes,
    X = singleStx, Y = singleMet, Cx = singleStxCoords,
...
An other feature is found most significant by GAMs, again with a negative relationship. For GAMs, we can also plot the corresponding spline fit, with the contributions to the correlation `r round(gamRes$result[gpGam<- rownames(gamRes$result)[1], "corxy"], 3)`:

```{r plotGAM}
plotGAMsTopResults(gamRes,
    X = singleStx, Y = singleMet, Cx = singleStxCoords,
    Ey = singleMetCoords, families = FamiliesVicari
)
```

Another available analysis for single images are Gaussian processes (GPs). Yet fitting GPs and calculating the score tests statistics can be computation and memory intensive! Hence the code is shown, but not run:

```{r gp, eval = FALSE}
gpRes <- sbivar(singleStx, singleMet, singleStxCoords, singleMetCoords, method = "GPs")
head(gpRes$result)
```
...
Next we plug the calculated Moran's I values into a linear model, with random effects for the individual mice, and adding a bogus fixed variable for illustrative purposes:
...
For the multi-image case, there are several options. One is to provide a list of _SpatialExperiment_ objects to the _sbivar_ function. We construct such list here for illustrative purposes, and fit GAMs:
